...
import { SettingsRaw } from '../../models/server/raw/Settings';
...
export class SettingsRegistry {
	private model: SettingsRaw;

	private store: ICachedSettings;

	private _sorter: {[key: string]: number} = {};

	constructor({ store, model }: { store: ICachedSettings; model: SettingsRaw }) {
		this.store = store;
		this.model = model;
	}
	...
	add(_id: string, value: SettingValue, { sorter, section, group, ...options }: ISettingAddOptions = {}): void {
		...
		if (settingStored && isOverwritten) {
			if (settingStored.value !== settingOverwritten.value) {
				this.model.update({ _id }, settingProps, {
					upsert: true,
				});
			}
			return;
		...
		this.model.insertOne(setting); // no need to emit unless we remove the oplog
	...
	*/
	...
	addGroup(_id: string, groupOptions: ISettingAddGroupOptions | addGroupCallback = {}, cb?: addGroupCallback): void {
		...
		if (!this.store.has(_id)) {
			options.ts = new Date();
			this.model.insertOne(options as ISetting);
