...
Meteor.startup(() => {
	...
	settings.watch('Assets_SvgFavicon_Enable', (value) => {
		...
		if (value) {
			injectIntoHead('Assets_SvgFavicon_Enable',
				`${ standardFavicons }
...
const renderDynamicCssList = _.debounce(Meteor.bindEnvironment(() => {
	...
	const colors = Promise.await(Settings.find({ _id: /theme-color-rc/i }, { fields: { value: 1, editor: 1 } }).toArray()).filter((color) => color && color.value);
