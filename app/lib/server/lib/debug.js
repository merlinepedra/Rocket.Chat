...
const wrapMethods = function(name, originalHandler, methodsMap) {
	methodsMap[name] = function(...originalArgs) {
		...
		logRequest(this.connection?.clientAddress, this.userId, method, this.connection?.httpHeaders.referer, this.connection?.httpHeaders['user-agent'], omitKeyArgs(args, name));
