 ...
 */
export class Democracy extends EventEmitter {
	/**
   ...
   */
	start() {
		...
		this.socket.bind(this.options.source[1], this.options.source[0], () => {
			...
			// Register on redis we're live
			const data = { key: process.env.CLUSTER_KEY, ...this.options };
			data.source = data.source.join(':');
			pub.hset('$rc/cluster-info', data.id, JSON.stringify(data));

			// Start sending 'hello' messages to the other nodes.
			this.hello();
		});

		redis.subscribe('$rc/cluster', function(err/*, count*/) {
			if (err) {
				throw err;
			}
		});

		pub.hgetall('$rc/cluster-info', (err, list) => {
			// console.log('list ->', list);
			Object.values(list).forEach((node) => {
				const data = JSON.parse(node);
				if (data.key === process.env.CLUSTER_KEY && data.id !== this.options.id) {
		...
		redis.on('message', (channel, message) => {
			...
			try {
				...
				if (data.key === process.env.CLUSTER_KEY) {
					this.addNodeToList(data);
				}
			} catch (e) {
   ...
   */
	hello() {
		...
		const data = { key: process.env.CLUSTER_KEY, ...this.options };
		data.source = data.source.join(':');
		pub.publish('$rc/cluster', JSON.stringify(data));
