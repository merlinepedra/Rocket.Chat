...
RocketChat.settings.get('Blockstack_Enable', (key, value) => {
	if (value) {
	// Blockstack login handler, triggered by a blockstack authResponse in route
		return Accounts.registerLoginHandler('blockstack', (loginRequest) => {
	...
	const index = Accounts._loginHandlers.findIndex(({ name }) => name === 'Blockstack_Enable');
	if (index > -1) {
		Accounts._loginHandlers.splice(index, 1);
	}
});
