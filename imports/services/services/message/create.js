...
export default {
	create: {
		...
		async handler(ctx) {
			...
			const { user, room, upsert = false } = ctx.params;
			...
			const prevent = await RocketChat.Services.call('apps.IPreMessageSentPrevent', { message });
			if (prevent) {
				throw new Meteor.Error('error-app-prevented-sending', 'A Rocket.Chat App prevented the message sending.');
			}

			let result = await RocketChat.Services.call('apps.IPreMessageSentExtend', { message });
			result = await RocketChat.Services.call('apps.IPreMessageSentModify', { message: result });
			...
			if (message) {
				...
				RocketChat.Services.call('apps.IPostMessageSent', { message });
